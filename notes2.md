## Answer to last homework (if and sums)
...
However, to call this function, we need to be able to show that the length of the argument vector is 
equal to `succ n` for some n.  This is ok if we know the length of the vector outright
...
show that the type `Vec Bool (succ 1)` is equal to the type `Vec Bool (plus 1 1)`.  
...
In our current simple language, that doesn't include arbitrary datatypes like vector, 
it's more difficult to observe this omission. But here is an example of some code that 
requires *beta-equivalence* to check.
...
expression with A). Given `A{true/x}`, `A{false/x}`, and `A{a/x}` (or anything
that they are definitionally equal to!) how can we figure out whether they
correspond?
...
And, in going to our bidirectional system, that can only happen 
...
You started proving things right away in Coq with an equality proposition. For example, 
in Coq, when you say 
...
This *type* is well-formed when both sides have the same type. In other words, it 
implements *homogeneous* equality.
...
Note that our elimination form for equality is still fairly powerful. We can use 
it to show that propositional equality is symmetric and transitive.
...
Complete the file [Hw2.pi](version2/test/Hw2.pi)
