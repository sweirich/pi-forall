...
parseExpr = testParser {- SOLN DATA -} initialConstructorNames{- STUBWITH -} expr
...
moduleImports = do
  ...
  return $ Module modName imports [] {- SOLN DATA -}
    initialConstructorNames{- STUBWITH -}
...
{- SOLN EQUAL -}
        ...
        mkArrowType  =
          do n <- Unbound.fresh wildcardName
             return $ \tyA tyB ->
               TyPi {- SOLN EP -} Rel {- STUBWITH -}tyA (Unbound.bind n tyB)
        mkTupleType =
          do n <- Unbound.fresh wildcardName
             return $ \tyA tyB ->
{- SOLN DATA -}
    ...
    do bd <- beforeBinder
       case bd of
         Colon (Var x) a ->
           option (Ann (Var x) a)
                  (do b <- afterBinder
                      return $ TyPi {- SOLN EP -} Rel {- STUBWITH -}a (Unbound.bind x b))
